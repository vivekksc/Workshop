...
namespace ServiceBus.Processor.Function
{
        ...
        )
    {
        ...
        private const string functionName = "BWProcessor";

        [Disable]
        [FunctionName(functionName)]
        public async Task RunAsync([TimerTrigger("%BWProcessorRunScheduleExpression%")] TimerInfo myTimer, ILogger log)
        {
            ...
            if (sessionTasks.Count > 0)
            {
                ...
                log.LogInformation($"{functionName} - {sessionTasks.Count} sessions processed.");
            }

            log.LogInformation($"{functionName} - No sessions found.");
        }
